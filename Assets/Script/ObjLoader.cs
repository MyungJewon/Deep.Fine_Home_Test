...
{      //구조체 선언
    ...
    public Verticesmap(string objname, List<Vector2> uv, List<int> triangles)
    {
        this.objname = objname; this.uv = uv; this.triangles = triangles;
    }
}
public class ObjImporter : MonoBehaviour
{
    public List<Verticesmap> LoadObj(string path){
        ...
        string objnametemp = "objnametemp";
        ...
        foreach (var line in lines)     //모든 데이터를 읽을때까지
        {
            ...
            if (line.StartsWith("g "))
            {
                if (objendcheck == true)
                {
                    Verticesmap oneverticesmap = new Verticesmap(objnametemp, uv, triangles);
        ...
        Verticesmap lastverticesmap = new Verticesmap(objnametemp, vertices, uv, triangles);
