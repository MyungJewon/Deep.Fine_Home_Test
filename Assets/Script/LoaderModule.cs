...
public class LoaderModule : MonoBehaviour
{
    ...
    {
        List<Verticesmap> loadedObject =await Task.Run(() => objImporter.LoadObjAsync(assetName));    //obj파일을 읽고 데이터만 저장
        GameObject objParent = new GameObject("ObjParent");

        //task에서 실행이 불가능한 Unity Api를 메인 threads에서 진행
        for(int i=0;i<loadedObject.Count;i++){
            Mesh mesh = new Mesh();
            mesh.vertices = loadedObject[i].vertices.ToArray();
            //mesh.uv = loadedObject[i].uv.ToArray(); //텍스처 불필요
            mesh.triangles = loadedObject[i].triangles.ToArray();
            mesh.RecalculateNormals(); // 법선 재계산

            // 메쉬 필터 컴포넌트에 메쉬 적용
            GameObject obj = new GameObject(loadedObject[i].objname);
